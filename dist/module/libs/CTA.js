import { i18nFormat } from "../../target-enhancements.js";
...
class CTArender {
    ...
    static async RenderAnim(tokenID, flagData, duplicate) {
        if (duplicate) {
            await CTArender.DeleteSpecificAnim, tokenID, duplicate.id;
        }
...
export class CTA {
    ...
    static AddTweens(token) {
        let testArray = [];
        if (token)
            testArray.push(token);
        else
            testArray = getCanvas().tokens.placeables;
    ...
    static addAnimation(token, textureData, pushActor, name, oldID) {
        if (typeof token === "string")
            token = getCanvas().tokens.get(token);
        if (!game.user.isGM) {
    ...
    static async removeAnim(token, animId, actorRemoval, fadeOut) {
        ...
        let actorFlags = Array.from(getProperty(token, "actor.data.token.flags.target-enhancements.anim") || []);
   ...
   */
    static async animationDialog(OGpath, token, oldData, name) {
        ...
        if (!token)
            token = getCanvas().tokens.controlled[0];
    ...
    static async getAnims(token) {
        if (getCanvas().tokens.controlled.length !== 1) {
            ui.notifications.notify(i18nFormat(MODULE_NAME + ".TokenError"));
            return;
        }
        if (!token)
            token = getCanvas().tokens.controlled[0];
...
Hooks.on("updateToken", (scene, token, update) => {
    if (!getProperty(update, "rotation"))
        return;
    let fullToken = getCanvas().tokens.get(token._id);
