...
BorderFrame.ToggleBorder = async function (event) {
...
BorderFrame.newBorder = function () {
    this.border.clear();
    ...
    if (game.settings.get(MODULE_NAME, "healthGradient")) {
        const stepLevel = game.settings.get(MODULE_NAME, "stepLevel");
        ...
        const endColor = hexToRGB(colorStringToHex(game.settings.get(MODULE_NAME, "healthGradientA")));
        const startColor = hexToRGB(colorStringToHex(game.settings.get(MODULE_NAME, "healthGradientB")));
    ...
    //@ts-ignore
        ...
        const p = game.settings.get(MODULE_NAME, "borderOffset");
...
BorderFrame.newBorderColor = function () {
    const overrides = {
        CONTROLLED: {
...
BorderFrame.newTarget = async function () {
    const multiplier = game.settings.get(MODULE_NAME, "targetSize");
