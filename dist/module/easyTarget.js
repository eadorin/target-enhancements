...
import { getCanvas, matchBoundKeyEvent, MODULE_NAME } from "./settings.js";
import { TargetEnhancements } from "./TargetEnhancements.js";
export class EasyTarget {
}
EasyTarget.getTemplateShape = function (template) {
...
EasyTarget.releaseOthersMap = new WeakMap();
// patch = function () {
// 	const releaseOthersMap = new WeakMap();
EasyTarget.tokenSetTarget = function (wrapped, ...args) {
...
EasyTarget.tokenOnClickLeft = function (wrapped, ...args) {
    const [event] = args;
...
EasyTarget.tokenCanControl = function (wrapped, ...args) {
...
EasyTarget.tokenLayerTargetObjects = function (wrapped, ...args) {
    const releaseOthers = EasyTarget.releaseOthersMap.get(this);
    if (releaseOthers !== undefined) {
        args[1].releaseOthers = releaseOthers;
    }
...
EasyTarget.canvasOnClickLeft = function (wrapped, ...args) {
    const [event] = args;
    const oe = event.data.originalEvent;
    const tool = ui.controls.control.activeTool;
...
EasyTarget.canvasOnDragLeftDrop = function (wrapped, ...args) {
    const [event] = args;
    const oe = event.data.originalEvent;
    const tool = ui.controls.control.activeTool;
...
EasyTarget.templateLayerOnDragLeftDrop = function (wrapped, ...args) {
    const [event] = args;
...
EasyTarget.keyboardManagerOnKeyC = function (wrapped, ...args) {
    const [event, up, modifiers] = args;
    if (!(modifiers.isShift && modifiers.isAlt)) {
        wrapped(...args);
    }
};
EasyTarget.tokenOnControl = function (wrapped, ...args) {
...
EasyTarget.releaseBehaviour = function (oe) {
...
EasyTarget.targetTokensInArea = function (templates, releaseOthers) {
    if (releaseOthers) {
        game.user.targets.forEach(token => token.setTarget(false, { releaseOthers: false, groupSelection: true }));
    }
    getCanvas().tokens.objects.children.filter((token) => {
